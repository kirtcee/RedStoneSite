...
import useBodyLock from "../hooks/useBodyLock"; // adjust path
...
function EntreesView({
  ...
  if (sub === "wings") {
    return (
      <div className="card" style={{ overflow: "visible", padding: "1rem 1rem 2rem" }}>
        {/* same “edge stretch” trick used on Specialty/Signature */}
        <div className="feast--edge">
          <WingsBuilder onClose={() => {}} onAdd={(item) => onAddToCart?.(item)} />
        </div>

        {/* scoped: only affects this card */}
        <style jsx>{`
          /* pull content out to the card’s edges so it lines up with the page header */
          .feast--edge {
  ...
  if (sub === "combos") {
    ...
    return (
      <div className="card" style={{ padding: "1rem 1rem 2rem" }}>
        ...
        {/* feast styles identical to your other sections */}
        <style jsx>{`
          ...
          .feastCard__img{ width:100%; height:100%; object-fit:fill; } /* stretch like Specialty */
          .feastCard__name{
  ...
  /* ---------- Special Menu: feast layout identical to Specialty/Signature ---------- */
  if (sub === "special") {
    ...
    return (
      <div className="card" style={{ padding: "1rem 1rem 2rem" }}>
        {/* feast grid sits directly in the .card */}
        <div className="feast feast--edge">
          <section className="feast__grid feast__grid--three">
            {cards.map((it) => (
        ...
        {/* Scoped styles — same as Specialty/Signature */}
        <style jsx>{`
          :global(:root) {
            --brand-blue: #006491;
            --red: #e91e28;
          ...
          .feastCard__img {
            ...
            width: 100%;
            height: 100%;
            object-fit: fill; /* stretch width, keep height 133px */
          }
          ...
          .btn {
            display: block;
            width: 100%;
            padding: 0.48rem 1.5rem; /* short height, wide buttons */
            font-family:var(--font-heading, Oswald, sans-serif);
            font-weight: 900;
        ...
        {selectedSpecialItem && (
          <>
            ...
            <div
              ...
              style={{ paddingTop: 24, paddingBottom: 24, overflowAnchor: 'none' }}  // 👈 add this
            >
...
/* ---------------- Drinks: feast grid + direct maroon modal --------------- */
function DrinksView() {
...
function MenuBody() {
  ...
  // Get cart info ONCE, then use it for totals
  const {
    addItem,
    openCart,
  ...
  const setTabSub = (next) => router.push({ pathname: "/menu", query: next }, undefined, { scroll: false, shallow: true });
  ...
  // <<< ONLY addition earlier: define the ref used below
...
{/* ===== Pizza Builder POPUP ONLY ===== */}
{showBuilder && (
  <>
    {/* soft white veil (not dark) */}
    <div
    ...
    <div
      className="modal-viewport modal-viewport--fixed"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="container" onClick={(e) => e.stopPropagation()}>
        <div className="modal-panel modal-panel--full">
          <button
                        type="button"
                        className="modal-close"
            onClick={(e) => {
          ...
          {/* no inner scroller! just a normal div */}
          <div>
            <PizzaBuilder
              presetToppings={presetToppings}
              initialData={editInitialData || {}}
              onSave={(payload) => {
                if (editingId) { /* optional remove old item */ }
                addItem(payload);
setEditInitialData(null);
setEditingId(null);
openCart();                         // lock stays on
requestAnimationFrame(() => setShowBuilder(false)); // close next frame
// open cart?
if (!DEBUG.disableOpenCart) {
  // open cart first, then close builder next frame (prevents scroll-lock flicker)
  openCart();
  requestAnimationFrame(() => setShowBuilder(false));
} else {
  setShowBuilder(false);
}
    ...
    {/* global styles for the new modal behavior */}
    <style jsx global>{`
      /* lock body when modal open handled by useBodyLock, but keep style parity */
      html { scrollbar-gutter: stable; }

      .modal-backdrop.modal-backdrop--soft {
        position: fixed; inset: 0;
        background: rgba(255,255,255,0.85); /* soft white veil */
        z-index: 2990;
      }
      .modal-viewport.modal-viewport--fixed {
        position: fixed; inset: 0;
        z-index: 3000;
      ...
      .modal-panel.modal-panel--full {
        position: relative;
        width: 100%;
        max-width: 1100px;                /* same content width as builder */
        background: #fff;
