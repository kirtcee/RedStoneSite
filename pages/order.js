...
function StoreMap({ onCarryoutClick, mapsReady, storeLatLng }) {
  ...
  }, [mapsReady, onCarryoutClick, storeLatLng]);

  // Update center/marker whenever storeLatLng changes
  useEffect(() => {
    const g = window.google;
    if (!mapsReady || !g || !mapObjRef.current || !markerRef.current) return;

    const pos = new g.maps.LatLng(storeLatLng.lat, storeLatLng.lng);
...
export default function OrderPage() {
  ...
  const handleConfirmDelivery = () => {
    ...
    try {
      ...
      localStorage.setItem("rs_delivery_address", JSON.stringify(payload));
    } catch {}
    router.push(router.query.next?.toString() || "/menu");
  };
